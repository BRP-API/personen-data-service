...
where {0} and natnltt.volg_nr = 0 and onjuist_ind is null";
		...
		public static (string where, IEnumerable<NpgsqlParameter> parameters) CreateBurgerservicenummerWhere(IEnumerable<string> bsns)
		{
			...
			return ($"where {where} and persoon_type = 'P' and pers.stapel_nr = 0 and pers.volg_nr = 0 and((pl.bijhouding_opschort_reden is not null and pl.bijhouding_opschort_reden != 'W' and pl.bijhouding_opschort_reden != 'F') or pl.bijhouding_opschort_reden is null)", parameters);
		}

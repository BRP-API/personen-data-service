...
public interface IGbaPersonenApiServiceDeprecated
{
	Task<(PersonenQueryResponse personenResponse, List<long>? plIds)> GetPersonen(PersonenQuery model);
}

public class GbaPersonenApiServiceDeprecated : BaseApiService, IGbaPersonenApiServiceDeprecated
{
	...
	public GbaPersonenApiServiceDeprecated(
	...
	{
		// Validation
		_fieldsExpandFilterService.ValidateScope(typeof(ApiModels.BRP.GbaPersoon), _persoonFieldsSettings.GbaFieldsSettings, model.fields);
        ...
        List<(ApiModels.BRP.GbaPersoon persoon, long pl_id)> result = new ();
        (IEnumerable<(ApiModels.BRP.GbaPersoon persoon, long pl_id)>? personenPlIds, int afnemerCode) = await _getAndMapPersoonService.GetPersonenMapByBsns(model.burgerservicenummer, model.gemeenteVanInschrijving, fieldsToUseForAuthorisations);
		...
		var mappedResult = _mapper.Map<List<Generated.Deprecated.GbaPersoon>>(result.ConvertAll(gbaPersoon => gbaPersoon.persoon));

        return (new RaadpleegMetBurgerservicenummerResponse { Personen = mappedResult }, result.Select(x => x.pl_id).ToList());
	}
    ...
    private async Task<(List<T>? personen, List<long>? plIds)> GetPersonenBeperktBase<T>(PersonenQuery model, bool? inclusiefOverledenPersonen = false) where T : GbaPersoonBeperkt
	{
        ...
        List<(T persoon, long pl_id)> result = new();
        ...
        if (model?.fields?.Any() == true && personenPlIds != null)
        {
            ...
            foreach (var x in personenPlIds.Where(x => x.persoon != null))
			{
				if (x.persoon is ApiModels.BRP.IPersoonMetGezag)
				{
					_gezagService.VerrijkPersonenMetGezagIfRequested(model.fields, gezag, gezagPersonen, ((ApiModels.BRP.IPersoonMetGezag persoon, long pl_id))x);
				}
	...
	private async Task<(ZoekMetGeslachtsnaamEnGeboortedatumResponse personenResponse, List<long>? plIds)> GetPersonenBeperkt(ZoekMetGeslachtsnaamEnGeboortedatum model)
	{
		...
		return (new ZoekMetGeslachtsnaamEnGeboortedatumResponse { Personen = _mapper.Map<List<Generated.Common.GbaPersoonBeperkt>>(personenPlIds.personen) }, personenPlIds.plIds);
	}
	...
	private async Task<(ZoekMetNaamEnGemeenteVanInschrijvingResponse personenResponse, List<long>? plIds)> GetPersonenBeperkt(ZoekMetNaamEnGemeenteVanInschrijving model)
	{
		...
		// Get personen + Filter response by fields
		return (new ZoekMetNaamEnGemeenteVanInschrijvingResponse { Personen = _mapper.Map<List<Generated.Common.GbaPersoonBeperkt>>(personenPlIds.personen) }, personenPlIds.plIds);
	}
	...
	private async Task<(ZoekMetNummeraanduidingIdentificatieResponse personenResponse, List<long>? plIds)> GetPersonenBeperkt(ZoekMetNummeraanduidingIdentificatie model)
	{
		...
		// Get personen + Filter response by fields
		return (new ZoekMetNummeraanduidingIdentificatieResponse { Personen = _mapper.Map<List<Generated.Common.GbaPersoonBeperkt>>(personenPlIds.personen) }, personenPlIds.plIds);
	}
	...
	private async Task<(ZoekMetPostcodeEnHuisnummerResponse personenResponse, List<long>? plIds)> GetPersonenBeperkt(ZoekMetPostcodeEnHuisnummer model)
	{
		...
		// Get personen + Filter response by fields
		return (new ZoekMetPostcodeEnHuisnummerResponse { Personen = _mapper.Map<List<Generated.Common.GbaPersoonBeperkt>>(personenPlIds.personen) }, personenPlIds.plIds);
	}
	...
	private async Task<(ZoekMetStraatHuisnummerEnGemeenteVanInschrijvingResponse personenResponse, List<long>? plIds)> GetPersonenBeperkt(ZoekMetStraatHuisnummerEnGemeenteVanInschrijving model)
	{
		...
		// Get personen + Filter response by fields
		return (new ZoekMetStraatHuisnummerEnGemeenteVanInschrijvingResponse { Personen = _mapper.Map<List<Generated.Common.GbaPersoonBeperkt>>(personenPlIds.personen) }, personenPlIds.plIds);
	}
	...
	{
		var personenPlIds = await GetPersonenBeperktBase<ApiModels.BRP.GbaGezagPersoonBeperkt>(model, model.inclusiefOverledenPersonen);

		// Get personen + Filter response by fields
		return (new ZoekMetAdresseerbaarObjectIdentificatieResponse { Personen = _mapper.Map<List<Generated.Deprecated.GbaGezagPersoonBeperkt>>(personenPlIds.personen) }, personenPlIds.plIds);
	}

...
public class GbaPersonenApiServiceDeprecated : BaseApiService, IGbaPersonenApiServiceDeprecated
{
	...
	private readonly IGezagServiceDeprecated _gezagService;
	...
	public GbaPersonenApiServiceDeprecated(
		...
		IGezagServiceDeprecated gezagService)
	...
	private async Task<(RaadpleegMetBurgerservicenummerResponse personenResponse, List<long>? plIds)> GetPersonen(RaadpleegMetBurgerservicenummer model)
	{
        ...
        (IEnumerable<(GbaPersoon persoon, long pl_id)>? personenPlIds, int _) = await _getAndMapPersoonService.GetPersonenMapByBsns(model.burgerservicenummer, model.gemeenteVanInschrijving, fieldsToUseForAuthorisations);

...
public class RvIGDataPersonenMapper(ICurrentDateTimeProvider currentDateTimeProvider, IDomeinTabellenHelper domeinTabellenHelper)
    : RvIGDataMapperBase(domeinTabellenHelper), IRvIGDataPersonenMapper
{
    private const string NATIONALITEIT = "Nationaliteit";
    ...
    private async Task MapGbaPersoonProperty(DbPersoonActueelWrapper dbPersoonWrapper, lo3_pl_persoon dbPersoon, GbaPersoon gbaPersoon, short? indicatieGeheim, string propertyName)
    {
        switch (propertyName)
        {
            ...
            case nameof(GbaPersoon.UitsluitingKiesrecht):
                MapUitsluitingKiesrecht(dbPersoonWrapper, gbaPersoon);
                break;
            case nameof(GbaPersoon.EuropeesKiesrecht):
                MapEuropeesKiesrecht(dbPersoonWrapper, gbaPersoon);
                break;
            ...
            case nameof(GbaPersoon.Verblijfplaats):
                await MapVerblijfplaats(dbPersoonWrapper, gbaPersoon);
                break;
            ...
            case nameof(GbaPersoon.GemeenteVanInschrijving):
                await MapGemeenteVanInschrijving(dbPersoonWrapper, gbaPersoon);
                break;
            case nameof(GbaPersoon.DatumInschrijvingInGemeente):
                MapDatumInschrijvingInGemeente(dbPersoonWrapper, gbaPersoon);
                break;
            case nameof(GbaPersoon.IndicatieCurateleRegister):
                MapIndicatieCurateleRegister(dbPersoonWrapper, gbaPersoon);
                break;
            case nameof(GbaPersoon.GezagInOnderzoek):
                MapGezagInOnderzoek(dbPersoonWrapper, gbaPersoon);
                break;
            case nameof(GbaPersoon.IndicatieGezagMinderjarige):
                await MapIndicatieGezagMinderjarige(dbPersoonWrapper, gbaPersoon);
                break;
            case nameof(GbaPersoon.Verblijfstitel):
                await MapVerblijfstitel(dbPersoonWrapper, gbaPersoon);
                break;
            case nameof(GbaPersoon.GeheimhoudingPersoonsgegevens):
                MapGeheimhouding(gbaPersoon, indicatieGeheim);
                break;
    ...
    private static void MapGeheimhouding(GbaPersoon gbaPersoon, short? indicatieGeheim)
    {
        gbaPersoon.GeheimhoudingPersoonsgegevens = indicatieGeheim.HasValue && indicatieGeheim != 0 ? indicatieGeheim : null;
    }

    private async Task MapVerblijfstitel(DbPersoonActueelWrapper dbPersoonWrapper, GbaPersoon gbaPersoon)
    {
        var verblijfstitel = await MapVerblijfstitel(dbPersoonWrapper!.Verblijfstitel);
        gbaPersoon.Verblijfstitel = GbaMappingHelper.IsDatumOnvolledigNullOrFutureDate(verblijfstitel?.DatumEinde, currentDateTimeProvider)
                                            && verblijfstitel?.Aanduiding?.Code?.Equals("98") == false
                                        ? verblijfstitel
                                        : null;
    }

    private async Task MapIndicatieGezagMinderjarige(DbPersoonActueelWrapper dbPersoonWrapper, GbaPersoon gbaPersoon)
    {
    ...
    private static void MapGezagInOnderzoek(DbPersoonActueelWrapper dbPersoonWrapper, GbaPersoon gbaPersoon)
    {
    ...
    private static void MapIndicatieCurateleRegister(DbPersoonActueelWrapper dbPersoonWrapper, GbaPersoon gbaPersoon)
    {
        if (dbPersoonWrapper?.Gezagsverhouding != null)
        {
    ...
    private static void MapDatumInschrijvingInGemeente(DbPersoonActueelWrapper dbPersoonWrapper, GbaPersoon gbaPersoon)
    {
    ...
    private async Task MapGemeenteVanInschrijving(DbPersoonActueelWrapper dbPersoonWrapper, GbaPersoon gbaPersoon)
    {
    ...
    private async Task MapVerblijfplaats(DbPersoonActueelWrapper dbPersoonWrapper, GbaPersoon gbaPersoon)
    {
    ...
    private async Task<(List<GbaPartner>? currentPartners, List<GbaPartner>? historicPartners)> GetPartners(List<lo3_pl_persoon> dbPartners)
    {
        ...
        {
            var actualPartner = currentPartnersOrdered[0];
    ...
    private static bool IsOuderNonExistent(GbaOuder ouder)
    {
    ...
    private static bool IsKindNonExistent(GbaKind kind)
    {
        return string.IsNullOrWhiteSpace(kind.Burgerservicenummer) &&
				kind.Naam == null &&
				kind.Geboorte == null;
    }

    private void MapEuropeesKiesrecht(DbPersoonActueelWrapper dbPersoonWrapper, GbaPersoon gbaPersoon)
    {
            ...
            switch (propertyName)
            {
                ...
                case nameof(GbaEuropeesKiesrecht.EinddatumUitsluiting):
                    if (inschrijving.europees_kiesrecht_aand == 1)
                    {
                        ...
                        if (GbaMappingHelper.IsDatumOnvolledigNullOrFutureDate(datumEinde, currentDateTimeProvider))
                        {
    ...
    private void MapUitsluitingKiesrecht(DbPersoonActueelWrapper dbPersoonWrapper, GbaPersoon gbaPersoon)
    {
        ...
        foreach (var propertyName in ObjectHelper.GetPropertyNames<GbaUitsluitingKiesrecht>())
        {
            switch (propertyName)
            {
                ...
                case nameof(GbaUitsluitingKiesrecht.Einddatum):
                    ...
                    if (GbaMappingHelper.IsDatumOnvolledigNullOrFutureDate(datum, currentDateTimeProvider))
                    {
    ...
    protected static GbaVerificatie? MapVerificatie(lo3_pl inschrijving)
    {
